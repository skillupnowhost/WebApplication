...
/** Condensed single-A4-page Jathagam, PROFESSIONAL style — clean, computer-typed, letterhead
 * treatment. Same data/sections as JathagamSinglePage (traditional), just without the handwritten
 * motifs and ornamentation. */
export function JathagamSinglePageProfessional({ view }: { view: ReportView }) {
  ...
  return (
    <>
      <ReportMasthead
        style="PROFESSIONAL"
        lang={lang}
        icon={AnimatedAscendantGlyph}
        compact
        kicker={`MyLoginn Astrology · ${AYANAMSA_NAMES[view.system]}`}
        ...
        meta={
          <>
            ...
            <p>
              {l("generatedL", lang)} · {DateTime.fromISO(view.createdAt, { setZone: true }).setLocale(locale).toFormat("dd MMM yyyy")}
            </p>
          </>
        }
        chips={
      ...
      <ReportSection title={`${l("personal", lang)} · ${l("birthDetails", lang)}`} style="PROFESSIONAL" lang={lang} onepage>
        <div className="jd-card jd-soft grid grid-cols-3 gap-x-3 gap-y-1.5 p-2 sm:grid-cols-4">
          <ReportField style="PROFESSIONAL" lang={lang} label={l("name", lang)} value={view.fullName} />
          <ReportField style="PROFESSIONAL" lang={lang} label={l("gender", lang)} value={view.gender ? lv(lang, cap(view.gender)) : "—"} />
          <ReportField style="PROFESSIONAL" lang={lang} label={l("dob", lang)} value={birthLocal.toFormat("dd MMM yyyy")} />
          <ReportField style="PROFESSIONAL" lang={lang} label={l("tob", lang)} value={view.birthTimeKnown ? birthLocal.toFormat("hh:mm a") : lv(lang, "Unknown (12:00 assumed)")} />
          <ReportField style="PROFESSIONAL" lang={lang} label={l("place", lang)} value={view.birthPlace} />
          <ReportField style="PROFESSIONAL" lang={lang} label={l("systemL", lang)} value={AYANAMSA_NAMES[view.system]} />
          <ReportField style="PROFESSIONAL" lang={lang} label={l("janmaRasi", lang)} value={rashiLabel(moon.rashi.index, lang)} />
          <ReportField style="PROFESSIONAL" lang={lang} label={l("nakPada", lang)} value={`${nakshatraLabel(moon.nakshatra, lang)} · ${moon.nakshatra.pada}`} />
          <ReportField style="PROFESSIONAL" lang={lang} label={l("lagnaAsc", lang)} value={rashiLabel(view.ascendant.rashi.index, lang)} />
          <ReportField style="PROFESSIONAL" lang={lang} label={l("sunSign", lang)} value={rashiLabel(sun.rashi.index, lang)} />
          <ReportField style="PROFESSIONAL" lang={lang} label={l("ganaL", lang)} value={lv(lang, view.avakahada.gana)} />
          <ReportField style="PROFESSIONAL" lang={lang} label={l("nadiL", lang)} value={lv(lang, view.avakahada.nadi)} />
        </div>
      </ReportSection>

      {/* ---- Charts ---- */}
      <ReportSection title={l("charts", lang)} style="PROFESSIONAL" lang={lang} onepage className="jd-charts">
        <div className="grid grid-cols-2 gap-2">
          {[
            ...
            <div key={c.title} className="jd-card p-1.5">
      ...
      {/* ---- Planetary positions ---- */}
      <ReportSection title={l("planetary", lang)} style="PROFESSIONAL" lang={lang} onepage>
      ...
      <div className="grid grid-cols-2 gap-3">
        <ReportSection title={l("dasha", lang)} style="PROFESSIONAL" lang={lang} onepage>
        ...
        <ReportSection title={l("upagrahaL", lang)} style="PROFESSIONAL" lang={lang} onepage>
          <div className="jd-card p-2">
      ...
      {/* ---- Lucky essentials ---- */}
      <ReportSection title={l("lucky", lang)} style="PROFESSIONAL" lang={lang} onepage>
        <div className="jd-card jd-soft grid grid-cols-4 gap-x-3 gap-y-2 p-2.5">
          <ReportField style="PROFESSIONAL" lang={lang} label={l("luckyNumbers", lang)} value={view.lucky.numbers.slice(0, 5).join(", ")} />
          <ReportField style="PROFESSIONAL" lang={lang} label={l("luckyColour", lang)} value={lv(lang, view.numerology.luckyColor)} />
          <ReportField style="PROFESSIONAL" lang={lang} label={l("luckyDay", lang)} value={lv(lang, view.lucky.day)} />
          <ReportField style="PROFESSIONAL" lang={lang} label={l("deityL", lang)} value={lv(lang, view.lucky.deity)} />
        </div>
      </ReportSection>

      {/* ---- Narrative headline ---- */}
      {view.narrative.headline && (
        <ReportSection title={l("summary", lang)} style="PROFESSIONAL" lang={lang} onepage className="mb-1.5">
          <div className="jd-card jd-soft p-2.5">
            <p className="text-[10px] leading-4 text-[var(--jd-ink)]">{view.narrative.headline}</p>
          </div>
        </ReportSection>
      )}

      <ReportFooter disclaimer={l("footerDisclaimer", lang)} className="mt-2 pt-2" />
    </>
  );
}
